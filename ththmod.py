...
def thth_map2(SS, tau, fd, eta1, edges1,eta2,edges2):
    """Map from Secondary Spectrum to theta-theta space
...
#     th_cents2 -= th_cents2[np.abs(th_cents2) == np.abs(th_cents2).min()]
    ...
    #print ('pntsshape', pnts.shape)
    
    #print ('ththpntsshape', thth[pnts].shape)
    #print ('pntsshape', pnts.shape)
    #print ('ththpntsshape', fd_inv[pnts])
    thth[pnts] = SS[1:,1:][tau_inv[pnts], fd_inv[pnts]]

    # Preserve flux (int
    thth *= np.sqrt(np.abs(2*eta1*th1-2*eta2*th2)).value
    return thth

def thth_redmap2(SS, tau, fd, eta1, edges1,eta2,edges2):
...
def rev_map2(thth, tau, fd, eta1, edges1, eta2, edges2,isdspec=True):
    """
    Map back from theta-theta space to SS space
    ...
    fd_map=(th_cents1[np.newaxis,:]-th_cents2[:,np.newaxis])
    tau_map=eta1.value*(th_cents1[np.newaxis,:]**2)-eta2.value*(th_cents2[:,np.newaxis]**2)
    fd_edges=(np.linspace(0,fd.shape[0],fd.shape[0]+1)-.5)*(fd[1]-fd[0]).value+fd[0].value
    tau_edges=(np.linspace(0,tau.shape[0],tau.shape[0]+1)-.5)*(tau[1]-tau[0]).value+tau[0].value
    ...
    recov=np.histogram2d(np.ravel(fd_map),
                         np.ravel(tau_map),
                         bins=(fd_edges,tau_edges),
                         weights=np.ravel(thth/np.sqrt(np.abs(2*eta1*th_cents1[np.newaxis,:]-2*eta2*th_cents2[:,np.newaxis]).value)).real)[0] +\
            np.histogram2d(np.ravel(fd_map),
                         np.ravel(tau_map),
                         bins=(fd_edges,tau_edges),
                         weights=np.ravel(thth/np.sqrt(np.abs(2*eta1*th_cents1[np.newaxis,:]-2*eta2*th_cents2[:,np.newaxis]).value)).imag)[0]*1j
    norm=np.histogram2d(np.ravel(fd_map),
                         np.ravel(tau_map),
                         bins=(fd_edges,tau_edges))[0]
    if isdspec:
...
def mosaic_ds(chunks):
    nct=chunks.shape[1]
    ncf=chunks.shape[0]
    cwf=chunks.shape[2]
    cwt=chunks.shape[3]
    ds_full=np.zeros(((ncf-1)*(cwf//2)+cwf,(nct-1)*(cwt//2)+cwt),dtype=float)

    for cf in range(ncf):
        for ct in range(nct):
            chunk_new=chunks[cf,ct,:,:]
            ds_full[cf*cwf//2:cf*cwf//2+cwf,ct*cwt//2:ct*cwt//2+cwt]=chunk_new
    return ds_full
